<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="字典名称" min-width="120px">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="字典编码" min-width="120px">
        <template slot-scope="{row}">
          <span>{{ row.code }}</span>
        </template>
      </el-table-column>
      <el-table-column label="字典描述" width="100" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" style="margin-top:-100px;">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="70px" style="width: 85%; margin-left:50px;">
        ...
        <el-form-item label="字典描述">
          <el-input v-model="temp.remark" placeholder="请输入字典描述" />
        </el-form-item>
      </el-form>
...
<script>
 import { getDictionaries, saveDictionary, deleteDictionary } from '@/api/dictionary'
